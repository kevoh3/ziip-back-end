...
class ExchangeMoneyController extends ApiController
{
    ...
    public function submitExchange(Request $request)
    {
        ...
        return DB::transaction(function () use ($request, $fromWallet, $charge) {
            // Re-fetch with locks to prevent race conditions
            $fromWallet = Wallet::where('id', $fromWallet->id)->lockForUpdate()->first();

            $toWallet = Wallet::where('currency_id',$request->to_currency_id)->where('user_id',auth()->id())->where('user_type',1)->lockForUpdate()->first();
